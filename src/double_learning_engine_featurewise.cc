...
void DoubleLearningEngineFeaturewise::init_knobs()
{
...
void DoubleLearningEngineFeaturewise::init_stats()
{

}

DoubleLearningEngineFeaturewise::DoubleLearningEngineFeaturewise(Prefetcher *parent, float alpha, float gamma, float epsilon, uint32_t actions, uint64_t seed, std::string policy, std::string type, bool zero_init)
	: LearningEngineBase(parent, alpha, gamma, epsilon, actions, 0 /*dummy state value*/, seed, policy, type)
{
	...
	{
		m_feature_knowledges[0][index] = NULL;
		m_feature_knowledges[1][index] = NULL;
	}
	for(uint32_t index = 0; index < knob::le_featurewise_active_features.size(); ++index)
	{
		...
		m_feature_knowledges[0][knob::le_featurewise_active_features[index]] = new FeatureKnowledge((FeatureType)knob::le_featurewise_active_features[index],
																							alpha,
																							gamma,
																							actions,
		...
		m_feature_knowledges[1][knob::le_featurewise_active_features[index]] = new FeatureKnowledge((FeatureType)knob::le_featurewise_active_features[index],
																							alpha,
																							gamma,
																							actions,
																							knob::le_featurewise_feature_weights[index],
		...
		assert(m_feature_knowledges[0][knob::le_featurewise_active_features[index]]);
		assert(m_feature_knowledges[1][knob::le_featurewise_active_features[index]]);
	}
...
DoubleLearningEngineFeaturewise::~DoubleLearningEngineFeaturewise()
{
	for(uint32_t index = 0; index < NumFeatureTypes; ++index)
	{
		if(m_feature_knowledges[0][index])
			delete m_feature_knowledges[0][index];
		if(m_feature_knowledges[1][index])
			delete m_feature_knowledges[1][index];
	}
}

uint32_t DoubleLearningEngineFeaturewise::chooseAction(State *state, float &max_to_avg_q_ratio, vector<bool> &consensus_vec)
{
	stats.action.called++;
	uint32_t action = 0;
...
void DoubleLearningEngineFeaturewise::learn(State *state1, uint32_t action1, int32_t reward, State *state2, uint32_t action2, vector<bool> consensus_vec, RewardType reward_type)
{
	stats.learn.called++;
	if(m_type == LearningType::SARSA && m_policy == Policy::EGreedy)
	{
		for(uint32_t index = 0; index < NumFeatureTypes; ++index)
		{
			if(m_feature_knowledges[qv_index][index])
			{
				if(!knob::le_featurewise_selective_update || consensus_vec[index])
				{
					m_feature_knowledges[qv_index][index]->updateQ(state1, action1, reward, state2, action2);
				}
...
uint32_t DoubleLearningEngineFeaturewise::getMaxAction(State *state, float &max_q, float &max_to_avg_q_ratio, vector<bool> &consensus_vec)
{
...
float DoubleLearningEngineFeaturewise::consultQ(State *state, uint32_t action)
{
	...
	for(uint32_t index = 0; index < NumFeatureTypes; ++index)
	{
		if(m_feature_knowledges[qv_index][index])
		{
			...
			{
				q_value += m_feature_knowledges[qv_index][index]->retrieveQ(state, action);
			}
			else if(knob::le_featurewise_pooling_type == 2) /* max pooling */
			{
				float tmp = m_feature_knowledges[qv_index][index]->retrieveQ(state, action);
				if(tmp >= max)
				{
					max = tmp;
					q_value = tmp;
...
void DoubleLearningEngineFeaturewise::dump_stats()
{
	...
	for(uint32_t index = 0; index < NumFeatureTypes; ++index)
	{
		if(m_feature_knowledges[qv_index][index])
		{
			fprintf(stdout, "learning_engine_featurewise.feature_%s_min_weight %0.8f\n", FeatureKnowledge::getFeatureString((FeatureType)index).c_str(), m_feature_knowledges[qv_index][index]->get_min_weight());
			fprintf(stdout, "learning_engine_featurewise.feature_%s_max_weight %0.8f\n", FeatureKnowledge::getFeatureString((FeatureType)index).c_str(), m_feature_knowledges[qv_index][index]->get_max_weight());
		}
	}
...
void DoubleLearningEngineFeaturewise::gather_stats(float max_q, float max_to_avg_q_ratio)
{
...
void DoubleLearningEngineFeaturewise::action_selection_consensus(State *state, uint32_t selected_action, vector<bool> &consensus_vec)
{
	stats.consensus.total++;
	...
	for(uint32_t index = 0; index < NumFeatureTypes; ++index)
	{
		if(m_feature_knowledges[qv_index][index])
		{
			if(m_feature_knowledges[qv_index][index]->getMaxAction(state) == selected_action)
			{
...
void DoubleLearningEngineFeaturewise::adjust_feature_weights(vector<bool> consensus_vec, RewardType reward_type)
{
	...
	for(uint32_t index = 0; index < NumFeatureTypes; ++index)
	{
		...
		{
			assert(m_feature_knowledges[qv_index][index]);
			...
			{
				m_feature_knowledges[qv_index][index]->increase_weight();
			}
			else if (isRewardIncorrect(reward_type))
			{
				m_feature_knowledges[qv_index][index]->decrease_weight();
			}
...
bool DoubleLearningEngineFeaturewise::do_fallback(State *state)
{
...
void DoubleLearningEngineFeaturewise::plot_scores()
{
	Scooby *scooby = (Scooby*)m_parent;
