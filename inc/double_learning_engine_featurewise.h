#ifndef DOUBLE_LEARNING_ENGINE_FEATUREWISE_H
#define DOUBLE_LEARNING_ENGINE_FEATUREWISE_H

#include <random>
#include "learning_engine_base.h"
#include "feature_knowledge.h"

class DoubleLearningEngineFeaturewise : public LearningEngineBase
{
private:
	FeatureKnowledge* m_feature_knowledges[2][NumFeatureTypes];
	int qv_index = 0;
	float m_max_q_value;
...
public:
	DoubleLearningEngineFeaturewise(Prefetcher *p, float alpha, float gamma, float epsilon, uint32_t actions, uint64_t seed, std::string policy, std::string type, bool zero_init);
	~DoubleLearningEngineFeaturewise();
